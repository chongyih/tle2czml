...
SATELLITE_COLOR_MAP = {
    "DS-EO": [255, 0, 255, 255],
    "DS-SAR": [255, 200, 0, 255],
    "TELEOS-2": [0, 255, 255, 255],
...
def read_tles(tles: str, rgbs):
    ...
    for line in tles.splitlines():
        ...
        if i % 3 == 0:
            ...
            sat_name = raw_tle[0].rstrip()

            # Assign color based on mapping or default to next color from Colors
            assigned_color = DEFAULT_RGBA  # fallback default

            for key in SATELLITE_COLOR_MAP:
                if sat_name.startswith(key):
                    assigned_color = SATELLITE_COLOR_MAP[key]
                    break
            else:
                # Use the cycling Colors instance only if not mapped
                assigned_color = rgbs.get_next_color()

            sats.append(Satellite(raw_tle, tle_object, assigned_color))
            raw_tle = []
        i += 1
